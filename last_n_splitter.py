...
class LastNSplitter(Splitter):
    ...
    def _add_time_partition(self, interactions: PandasDataFrame) -> PandasDataFrame:
    ...
    def _to_unix_timestamp(self, interactions: PandasDataFrame) -> PandasDataFrame:
    ...
    def _to_unix_timestamp_pandas(self, interactions: PandasDataFrame) -> PandasDataFrame:
        ...
        if time_column_type == np.dtype("datetime64[ns]"):
            ...
            interactions[self.timestamp_column] = (interactions[self.timestamp_column] - pd.Timestamp("1970-01-01")
                                                  ) // pd.Timedelta("1s")
        return interactions

    def _partial_split_interactions(self, interactions: PandasDataFrame, n: int) -> Tuple[PandasDataFrame, PandasDataFrame]:
    ...
    def _core_split(self, interactions: PandasDataFrame) -> List[PandasDataFrame]:
        if self.strategy == "timedelta":
